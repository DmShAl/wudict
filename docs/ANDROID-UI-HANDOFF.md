...
| Window chrome and toolbar | `web/group-editor.css`: `#dictSettings` height and overscroll, `#dictSettings .configbar` and `#dictSettings #ftsAllBox` pinned (`flex:none`) so the actions stay reachable and the bulk box takes its own line |
