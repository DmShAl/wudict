...
- **Open-order strategy (D142)**: which accordion section auto-expands is the reader's choice, stored as `UIPrefs.orderFirst` in `state.json` and surfaced as `Open first: Fastest | My order` in the ☰ panel. **Fastest** (default, zero value) is D73's latch unchanged: the first dictionary to *answer* takes the open slot and keeps it. **My order** opens the first dictionary in the user's own order that has results — slot `i` may open only once every slot above it has reported and none of them had anything. Purely client-side: `/api/search` is untouched, because the slot layout is already emitted in preference order on `begin` before any dictionary is opened, so what changes is which section unfolds, never what is searched or in what order. Both strategies are one predicate (`settle()` in `index.html`) over a per-generation resolution frontier, so they cannot drift apart; `deferred`, errored, skipped and empty slots *resolve* the frontier without ever becoming the section that opens. The intrinsic cost — time-to-open is `max()` over the prefix rather than `min()` over everything — is bounded by a 700 ms deadline (`OPEN_BARRIER_MS`) armed on `begin`: on expiry the best answer already in hand opens and the Fastest rule resumes, so the worst case is the default plus 700 ms. On a prepared library the frontier resolves in milliseconds and it never fires. D73's invariant is untouched under both: nothing that opens ever closes, moves or reopens under a reader.
