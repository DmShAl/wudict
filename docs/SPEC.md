...
- Triggered: CLI `wudict ingest [-full] [<path|dir>…]` (no path: the configured `DICT_DIR`; a plan flag left out keeps the prepared plan, D152); web UI per-dict "enable full-text search" / "index all" with SSE progress; headword-level indexing happens silently on first search (D13).
