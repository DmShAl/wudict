...
- For the state of the latest agent work session (branch, what changed and why, what remains, Windows verification recipes and known test failures), read `HANDOFF.md` in the repository root before planning anything. When the session ends, update `HANDOFF.md` in place — refresh the branch/commit state, move finished items out of "what remains", add new facts worth carrying over — but keep it small and do not turn it into a file-tree dump: anything derivable from the code with a grep does not belong there.
